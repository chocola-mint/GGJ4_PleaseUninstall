	...
	while i < pool_size:
		...
		if bullet.is_alive: 
			...
			# [Alive script]
				...
				- bullet.global_position).normalized() * pool_vel[i].length()
				var towards_desired_vel = (desired_vel - pool_vel[i]).normalized()
				pool_vel[i] += towards_desired_vel * delta * uniform_steer
				if pool_vel[i].length_squared() < uniform_speed * uniform_speed:
...
func update_nearest_enemy():
	...
	for node in get_tree().get_nodes_in_group("enemies"):
		...
		var dist = enemy.global_position.distance_squared_to(GameManager.player.body.global_position)
